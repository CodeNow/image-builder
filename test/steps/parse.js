    ...
    lab.experiment('valid', function () {
      [
        ...
        it('should add weave to correct line', function(done) {
          var dockerfile = parser(item).split('\n');
          expect(dockerfile[0])
            .to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
          expect(dockerfile[1])
            .to.contain(process.env.RUNNABLE_WAITFORWEAVE);
          expect(dockerfile[2])
            .to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
          done();
        });
      ...
      it('should add weave to both lines', function(done) {
        ...
        var dockerfile = parser(item).split('\n');
        expect(dockerfile[0]).to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[1]).to.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[2]).to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[3]).to.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[4]).to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
        done();
      });
      ...
      it('should not add weave to second part of run line', function(done) {
        ...
        expect(dockerfile[1]).to.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[2]).to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
        expect(dockerfile[3]).to.not.contain(process.env.RUNNABLE_WAITFORWEAVE);
        done();
      });
    });
