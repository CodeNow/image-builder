...
class VaultManager {
  constructor () {
    ...
    utils.log('here is the vaultpath', vaultPath)
    if (vaultPath) {
      try {
  ...
  readRegistryPassword () {
    ...
    return Bluebird.try(function () {
      if (!self._vault) {
        throw new Error('Error reading registry password - Vault was not configured')
      }
    })
      .then(function () {
  ...
  readUserSSHKey (userId) {
    ...
    return Bluebird.try(function () {
      if (!self._vault) {
        throw new Error('Error reading ssh keys - Vault was not configured')
      }
    })
      .then(function () {
