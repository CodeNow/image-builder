...
Builder.prototype.getRegistryConfig = function (cb) {
  ...
  if (registryConf.url && registryConf.username) {
    vault.readRegistryPassword()
    .then(function (password) {
      utils.log(JSON.stringify(password))
      utils.log('password2' + password.value)
      registryConf.password = password
      utils.log('Logging into ' + registryConf.url + ' as ' + registryConf.username + ' and ' + registryConf.password)
      cb(null, registryConf)
    })
    .catch(cb)
  } else {
    // do not return registry config
    cb(null)
  }
}
...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  this.getRegistryConfig(function (err, registryConf) {
    utils.log('resultssss' + err)
    if (err) {
      return cb(err)
    }
    if (registryConf) {
      utils.log('resultssss1' + registryConf)
      opts.registryconfig = registryConf
    }
    ...
    if (runBuildContext) {
      ...
      utils.log('Runnable absoulute build context: '+ absBuildContext);
      if (this.dockerfilePath.indexOf(absBuildContext) === 0) {
        opts.dockerfile = path.relative(absBuildContext, this.dockerfilePath);
      } else {
