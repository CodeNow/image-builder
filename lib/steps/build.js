...
Builder.prototype._handleBuildData = function (data) {
  ...
  try {
    ...
    // do not stop builds for parse error. treat like normal line
    // failed builds will be caught later
    data = {
      stream: data
    };
  }
  ...
  if (data.error) {
    self.buildErr = new Error(data.error);
    // do not print build errors
    self.buildErr.noLog = true;
