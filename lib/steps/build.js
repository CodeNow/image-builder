...
Builder.prototype.getLocalRegistryConfig = function () {
  return Promise.fromCallback(cb => fs.readFile('/root/.docker/config.json', cb))
...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  const errHandler = err => {
    utils.log('Error fetching registry credentials: ' + err.message)
    return null
  }
  ...
  .then(function (props) {
    const registryConf = props.registryConfAndSShKeys
    const localRegistryConf = props.localRegistryConfig
    if (registryConf || localRegistryConf) {
      opts.registryconfig = localRegistryConf || {}
      if (registryConf) {
        ...
        if (registryConf.sshKeyArgs && process.env.RUNNABLE_BUILD_DOCKERFILE) {
          opts.buildargs = registryConf.sshKeyArgs
        }
      }
    }
