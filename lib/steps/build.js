...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  // If there is a file docker.runnable we should pull it and parse it for the registry
  var configFile = path.join(steps.dirs.dockerContext, 'docker.runnable');
  var registryConf = null;
  if (fs.fileExistsSync(configFile)) {
    var rawContents = fs.readFileSyn(configFile, { encoding: 'utf-8' });
    try {
