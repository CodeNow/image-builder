...
function Builder (steps) {
  ...
  utils.log('Builder jorge buildRoot' + this.buildRoot);
  utils.log('Builder jorge dockerBuildLog' + this.dockerBuildLog);
  utils.log('Builder jorge dockerContext' + this.dockerContext);
  utils.log('Builder jorge RUNNABLE_DOCKER' + process.env.RUNNABLE_DOCKER);
...
Builder.prototype.runDockerBuild = function (cb) {
  var self = this;
  utils.log('runDockerBuild jorge 0' + self.buildRoot);
  var tarStream = self._getTarStream();
  var opts = {
    t: process.env.RUNNABLE_DOCKERTAG
  };
  utils.log('runDockerBuild jorge 1');
  ...
  utils.log('Builder buildImage jorge');
...
Builder.prototype._getTarStream = function () {
  utils.log('getTarStream jorge ' + this.buildRoot);
