...
if (process.env.RUNNABLE_WAITFORWEAVE) {
    ...
    .on('finish', function() {
      docker.buildImage(tarPath, {
        ...
        response.on('data', function(data) {
          ...
          } else if (~data.stream.indexOf(process.env.RUNNABLE_WAITFORWEAVE)) {
            out = data.stream.replace(process.env.RUNNABLE_WAITFORWEAVE, '');
            needAttach = true;
          }
