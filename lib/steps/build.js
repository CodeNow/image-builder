...
Builder.prototype.getRegistryConfig = function (cb) {
  ...
  if (!process.env.RUNNABLE_SSH_KEY_IDS && (!registryConf.url || !registryConf.username)) {
    return cb(null)
  }
  let vaultTasks = {}
  vaultTasks.sshKeyArgs = sshKeyReader.createSSHKeys()
  if (registryConf.url && registryConf.username) {
    vaultTasks.registryConf = vault.readRegistryPassword()
  }
  return Bluebird.props(vaultTasks)
    .then(function (resp) {
      if (resp.registryConf && resp.registryConf.data && resp.registryConf.data.value) {
        var password = resp.registryConf.data.value
        registryConf.password = password
        utils.log('Logging into ' + registryConf.url + ' as ' + registryConf.username)
        return { registryConf, sshKeyArgs: resp.sshKeyArgs }
      }
      return resp
    })
    .asCallback(cb)
...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  this.getRegistryConfig(function (err, registryConfAndSShKeys) {
    if (err) {
      utils.log('error fetching private registry credentials')
    }

    if (registryConfAndSShKeys) {
      if (registryConfAndSShKeys.registryConf) {
        opts.registryconfig = {}
        opts.registryconfig[registryConfAndSShKeys.registryConf.url] = {
          username: registryConfAndSShKeys.registryConf.username,
          password: registryConfAndSShKeys.registryConf.password
        }
      }
      // we cannot inject ssh keys into a non-repo container, so we need to test for the RUNNABLE_BUILD_DOCKERFILE var
