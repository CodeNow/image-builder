...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  if (process.env.RUNNABLE_BUILD_DOCKERFILE) {
    opts.dockerfile = process.env.RUNNABLE_BUILD_DOCKERFILE;
  }
  if (process.env.RUNNABLE_CUSTOM_REGISTRY_URL &&
    process.env.RUNNABLE_CUSTOM_REGISTRY_USERNAME &&
    process.env.RUNNABLE_CUSTOM_REGISTRY_PASSWORD
  ) {
    // Registry config documentation: https://docs.docker.com/engine/reference/api/docker_remote_api_v1.22/#build-image-from-a-dockerfile
    opts.registryconfig = {};
    opts.registryconfig[process.env.RUNNABLE_CUSTOM_REGISTRY_URL] = {
      username: process.env.RUNNABLE_CUSTOM_REGISTRY_USERNAME,
      password: process.env.RUNNABLE_CUSTOM_REGISTRY_PASSWORD
    };
  }
