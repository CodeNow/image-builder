...
Builder.prototype.getRegistryConfig = function () {
  ...
  return vault.readRegistryPassword()
    .then(function (resp) {
      if (resp && resp.data && resp.data.value) {
        var password = resp.data.value
...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  return Promise.props({
    registryConfig: self.getRegistryConfig().catch(errHandler),
    localRegistryConfig: self.getLocalRegistryConfig().catch(errHandler)
  })
  .then(function (props) {
    const registryConf = props.registryConfig
    const localRegistryConf = props.getLocalRegistryConfig
