...
Builder.prototype.runDockerBuild = function (cb) {
  ...
  if (runBuildContext) {
    var absoluteBuildContext = path.resolve('/', runBuildContext)
    utils.log('Runnable build context: ' + runBuildContext + ',' + absoluteBuildContext);
    utils.log('Index of:' + this.dockerfilePath.indexOf(absoluteBuildContext));
    if (this.dockerfilePath.indexOf(absoluteBuildContext) === 0) {
      opts.dockerfile = path.relative(absoluteBuildContext, this.dockerfilePath);
    } else {
