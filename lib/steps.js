  ...
  getRepositories: function (cb) {
    ...
    async.mapSeries(
      ...
      function (repo, cb) {
        ...
        var repoFullName;
        if (parsed.protocol) {
          repoFullName = parsed.pathname.slice(1);
          repoName = repoFullName.split('/').pop();
          authString = parsed.auth + '@';
          delete parsed.auth;
          noAuthUrl = url.format(parsed);
        } else {
          repoFullName = repo.split(':').pop();
          repoName = repoFullName.split('/').pop();
        }
        ...
        async.waterfall([
          ...
          function cleanRepoOfToken (cb) {
            if (noAuthUrl && authString) {
              childProcess.exec(
                'git remote set-url origin ' + noAuthUrl,
                {
                  cwd: repoTargetDir
                },
                steps.saveToLogs(function (err) {
                  if (err) { return cb(err); }
                  childProcess.exec(
                    'sed -i "s|' + authString + '||g" ' +
                      '$(grep -Rl "' + authString + '" .git)',
                    {
                      cwd: repoTargetDir
                    },
                    steps.saveToLogs(function (err) { cb(err); })
