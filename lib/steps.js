...
var steps = module.exports = {
  ...
  applyRepositoryFilters: function (cb) {
    ...
    async.map(repos, filterRepository, cb);

    function filterRepository(repo, cb) {
      var repoName = repo.name.split('/').pop();
      var repoFullName = repo.name.split(':').pop();
      var repoCacheDir = path.join(cacheDir, repoFullName);
      var repoTargetDir = path.join(steps.dirs.dockerContext, repoName);
      ...
      async.series([
        function renameFiles(cb) {
          async.map(repo.rename, function (rule, cb) {
            var source = [repoTargetDir, rule.source].join('/');
            var dest = [repoTargetDir, rule.dest].join('/');
            var command = ['mv', source, dest].join(' ');
            childProcess.exec(command, steps.saveToLogs(cb));
          }, cb);
        },
        function searchAndReplace(cb) {
          async.map(repo.replace, function (rule, cb) {
            var command = ['grep -rn', rule.search, repoTargetDir].join(' ');
