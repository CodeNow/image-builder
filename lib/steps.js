...
var steps = module.exports = {
  ...
  downloadBuildFiles: function (cb) {
    ...
    childProcess.execFile('node', ['downloadS3Files.js'].concat(args),
      {
        cwd: thisDir,
        maxBuffer: 1024 * 5000
      },
          ...
          async.waterfall([
            ...
            function cacheOrClone (lockAquired, cb) {
              if (lockAquired) {
                ...
                async.series([
                  function upsertCachedRepoDir (cb) {
                    if (fs.existsSync(path.join(repoCacheDir, '.git'))) {
                      ...
                      childProcess.execFile('git', ['clone', '-q', repo, repoCacheDir],
                        { maxBuffer: 1024 * 5000 },
                        steps.saveToLogs(cb));
                    }
                  },
                ...
                childProcess.execFile('git', ['-q', repo, repoTargetDir],
                  { maxBuffer: 1024 * 5000 },
                  steps.saveToLogs(function (err) {
                    if (err) { return cb(err); }
                    childProcess.execFile('git', ['checkout', '-q', commitish],
  ...
  copyLayer: function (cb) {
    ...
    var args = [
      ...
      '-v', process.env.DOCKER_IMAGE_BUILDER_LAYER_CACHE + ':' + layerCacheDir
    ];
    if (/docker.sock/.test(process.env.RUNNABLE_DOCKER)) {
      args.push('-v');
      args.push('/var/run/docker.sock:/var/run/docker.sock');
    }
    args.push(process.env.RUNNABLE_IMAGE_BUILDER_NAME + ':' + process.env.RUNNABLE_IMAGE_BUILDER_TAG);
  ...
  pushImage: function (cb) {
    ...
    args.push(process.env.RUNNABLE_IMAGE_BUILDER_TAG);
