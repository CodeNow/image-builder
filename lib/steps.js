...
var steps = module.exports = {
  ...
  parseDockerfile: function (cb) {
    ...
    steps.dockerfilePath = path.join(steps.dirs.buildRoot, 'Dockerfile');
    if (process.env.RUNNABLE_BUILD_DOCKERFILE) {
      var reposAndBranches = zip(
        process.env.RUNNABLE_REPO.split(';'),
        process.env.RUNNABLE_COMMITISH.split(';'));
      // there is only one repo if we are in this mode
      var repoName = Object.keys(reposAndBranches).reduce(function (repo) {
        return repo.split('/').pop();
      });
      steps.dirs.buildRoot = path.join(steps.dirs.dockerContext, repoName);
      steps.dockerfilePath = path.join(
        steps.dirs.buildRoot,
        repoName,
        process.env.RUNNABLE_BUILD_DOCKERFILE);
    }

    var dockerfile = fs.readFileSync(steps.dockerfilePath).toString();
    ...
    function saveDockerfile (cb) {
      fs.writeFileSync(steps.dockerfilePath, dockerfile);
      cb();
    }
  },
