...
var steps = module.exports = {
  ...
  getRepositories: function (cb) {
    ...
    childProcess.exec('mkdir -p ' + cacheDir, function (err) {
      ...
      var reposAndKeys = process.env.RUNNABLE_DEPLOYKEY ? zip(
        process.env.RUNNABLE_REPO.split(';'),
        process.env.RUNNABLE_DEPLOYKEY.split(';')) : null;
      ...
      async.mapSeries(
        ...
        function (repo, cb) {
          ...
          var repoKey = reposAndKeys ? reposAndKeys[repo] : null;
          var repoKeyPath = repoKey ? path.join(steps.dirs.keyDirectory, repoKey) : null;
  ...
  applySearchAndReplace: function (cb) {
    ...
    console.log('Applying search and replace rules.');
