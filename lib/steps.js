...
var steps = module.exports = {
  ...
  getRepositories: function (cb) {
    ...
    childProcess.execFile('mkdir', ['-p', cacheDir], function (err) {
      ...
      async.mapSeries(
        ...
        function (repo, cb) {
          steps.repoName = repo.split('/').pop();
          var repoFullName = repo.split(':').pop();
          var repoCacheDir = path.join(cacheDir, repoFullName);
          var repoTargetDir = path.join(steps.dirs.dockerContext, steps.repoName);
  ...
  parseDockerfile: function (cb) {
    steps.dirs.buildContext = steps.dirs.dockerContext;
    if (process.env.BUILD_CONTEXT) {
      steps.dirs.buildContext = path.join(
    ...
    var dockerfile =
      fs.readFileSync(path.join(steps.dirs.buildContext, 'Dockerfile'))
        .toString();
    ...
    function saveDockerfile (cb) {
      fs.writeFileSync(
        path.join(steps.dirs.buildContext, 'Dockerfile'),
        dockerfile);
      cb();
    }
  },
