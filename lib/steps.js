...
var steps = module.exports = {
  ...
  getRepositories: function (cb) {
    ...
    childProcess.exec('mkdir -p ' + cacheDir, function (err) {
      if (err) { return cb(err); }
        ...
        function (repo, cb) {
          ...
          async.waterfall([
            ...
            function addKeyToAgent (cb) {
              childProcess.exec('ssh-add -D', function (err) {
                if (err) { return cb(err); }
                childProcess.exec(
                  'ssh-add ' + repoKeyPath,
                  steps.saveToLogs(function (err) {
            ...
            function tryForLock (cb) {
              ...
              childProcess.exec(
                'mkdir -p ' + path.dirname(lockfileName),
                function (err) {
                  if (err) { return cb(err); }
              ...
              if (lockAquired) {
                ...
                async.series([
                  ...
                  function getHEADCommit (cb) {
                    childProcess.exec('git rev-parse HEAD',
                      {
                        cwd: repoCacheDir
                      },
                  ...
                  function updateCachedRepo (cb) {
                    if (fetchAndCheckout) {
                      childProcess.exec('git fetch --all',
                        {
                          cwd: repoCacheDir
                        },
                        steps.saveToLogs(cb)
                  ...
                  function checkoutCommitish (cb) {
                    if (fetchAndCheckout) {
                      childProcess.exec('git checkout -q ' + commitish,
                        {
                          cwd: repoCacheDir
                        },
                        steps.saveToLogs(cb)
                  ...
                  function copyToTargetDir (cb) {
                    ...
                    childProcess.exec('cp ' + args, steps.saveToLogs(cb));
                  },
  ...
  applyRepositoryFilters: function (cb) {
    if (!process.env.RUNNABLE_REPO_FILTER) {
      return cb();
    }
    else {
      try {
        var repos = JSON.parse(process.env.RUNNABLE_REPO_FILTER);
        async.each(repos, filterRepository, cb);
      }
      catch (err) {
        return cb(err);
      }
    ...
    function filterRepository(repo, cb) {
      var repoName = repo.name.split('/').pop();
      var repoDir = path.join(steps.dirs.dockerContext, repoName);

      async.series([
        function renameFiles(cbRenameFiles) {
          async.map(repo.rename, function (rule, cb) {
            var source = [repoDir, rule.source].join('/');
            var dest = [repoDir, rule.dest].join('/');
            var command = ['mv', source, dest].join(' ');
            childProcess.exec(command, steps.saveToLogs(cb));
          }, cbRenameFiles);
        },
        function searchAndReplace(cbSearchAndReplace) {
          async.each(repo.replace, function (rule, cbReplace) {
            ...
            childProcess.exec(command, function (err, result) {
              ...
              // 2. Remove blacklisted files from result set
              var blacklist = repo.blacklist[rule.search];
              files = files.filter(function (file) {
                return blacklist.reduce(function (memo, curr, index) {
                  if (curr.line) {
