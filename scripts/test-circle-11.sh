...
grep -q "waitForWeave" $build_log || (echo "waitForWeave should be added" && false)
...
# waitForWeave should NOT be in output
# awk will exit if it finds waitForWeave
awk '/waitForWeave/{exit 1}' $build_log || (echo "waitForWeave should NOT be added" && false)
